 ...
 *            `class_id x1 y1 x2 y2 ...` for a polygon (YOLO-seg), plus the
 *            class_id -> name mapping in BOTH conventions real tooling
 *            expects: classes.txt (Darknet/LabelImg/CVAT, and what this
 *            app's own importer reads back) and data.yaml (Ultralytics
 *            YOLOv5/v8+). No slot for line/sequence -- every real YOLO
 *            reader expects an exact, fixed token count per line, so
 *            inserting extra numbers would corrupt it for anyone else's
 *            trainer. Same "no label, can't represent, skipped + counted"
 *            limitation as COCO.
 *
 ...
 * zip, one file per image (YOLO's zip also carries the shared classes.txt
 * and data.yaml); COCO's bulk download is a single combined manifest, per
 * COCO convention.
 */
...
function orderedShapes(image) {
  const info = computeLineNumbers(image.annotations, image.lineRegions);
...
// ---------------- Pascal VOC (strict) ----------------
// Full canonical structure (folder/source/segmented at image level,
// occluded alongside pose/truncated/difficult per object) -- not just the
// minimum a lenient parser would accept. Some real VOC-reading tools parse
// the whole tree positionally/schema-strictly rather than tolerantly
// looking up just the tags they need, so matching the documented shape
// (per the original PASCAL devkit convention, e.g. as summarized at
// https://roboflow.com/formats/pascal-voc-xml) costs nothing and removes
// that risk. None of this app's own re-import logic reads these fields --
// only <name>/<bndbox>/<polygon> matter to LabelFlow itself -- so adding
// them is purely for compatibility with OTHER tools, not a functional
// change here.
function buildImageVoc(project, image) {
  const L = [];
  L.push(`<annotation>`);
  L.push(`  <folder>${esc(project.name)}</folder>`);
  L.push(`  <filename>${esc(image.fileName)}</filename>`);
  L.push(`  <source>`);
  L.push(`    <database>Unknown</database>`);
  L.push(`  </source>`);
  L.push(`  <size><width>${image.width}</width><height>${image.height}</height><depth>3</depth></size>`);
  L.push(`  <segmented>0</segmented>`);
...
function buildCocoManifest(project, images) {
  // supercategory is optional per the COCO spec (pycocotools doesn't
  // require it), but some stricter readers/validators expect the key to at
  // least exist -- "none" is the conventional default when there's no real
  // category hierarchy, same as most COCO-export tools use.
  const categories = project.labels.map((l, i) => ({ id: i + 1, name: l.name, supercategory: "none" }));
...
// classes.txt (one name per line, index = class_id) is the Darknet/LabelImg/
// CVAT convention -- and what this app's OWN importer reads back (see
// parseYoloClasses). data.yaml (below) is the newer Ultralytics YOLOv5/v8+
// convention, which reads class names from a `names:` map/list in a YAML
// config rather than a plain text file. Different tools expect different
// ones of the two, so both are included -- covers both without either side
// needing to hand-convert.
function buildClassesTxt(project) {
  return project.labels.map((l) => l.name).join("\n") + (project.labels.length ? "\n" : "");
}

function yamlStr(s) {
  // Quoted + escaped so any label name (colons, quotes, whatever) is safe
  // to drop straight into YAML without corrupting the file.
  return `"${String(s).replace(/\\/g, "\\\\").replace(/"/g, '\\"')}"`;
}

/** Minimal Ultralytics-style dataset config -- just the class list. Doesn't
 *  (can't) include train/val image folder paths: those depend on how the
 *  files get arranged after download, which LabelFlow has no visibility
 *  into -- only the class list is something it can correctly know. */
function buildDataYaml(project) {
  const lines = [
    "# Minimal Ultralytics-style dataset config -- class list only.",
    "# Add train/val image-folder paths once the exported files are arranged",
    "# into the images/ + labels/ layout Ultralytics expects.",
    `nc: ${project.labels.length}`,
    "names:",
  ];
  project.labels.forEach((l, i) => lines.push(`  ${i}: ${yamlStr(l.name)}`));
  return lines.join("\n") + "\n";
}
 ...
 *  conventions for that mapping (classes.txt and data.yaml). */
function buildYoloSingleZip(project, image) {
  ...
  return buildZip([
    ...
    { name: "data.yaml", data: buildDataYaml(project) },
  ]);
}
...
function buildExportZip(project, images, format, completedOnly, coords = "both") {
  ...
  // class_id -> name mapping -- bundle it once at the zip root (both
  // conventions, see buildClassesTxt) rather than repeating it per image.
  if (format === "yolo") {
    entries.push({ name: "classes.txt", data: buildClassesTxt(project) });
    entries.push({ name: "data.yaml", data: buildDataYaml(project) });
  }
...
module.exports = {
  ...
  buildYoloTxt, buildClassesTxt, buildDataYaml, buildYoloSingleZip,
