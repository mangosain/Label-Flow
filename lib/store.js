 ...
 *                                lineRegions: [{id, points, line}]}]
 *                                -- lineRegions are the freeform "fix line
 *                                numbers on a skewed page" lasso regions
 *                                (see lib/lines.js); not shapes, no label,
 *                                never exported as their own object.
...
// Same self-healing default as getImage() below, applied across the whole
// list -- getImages() feeds both the admin image list AND the exporter
// (which needs every image's lineRegions, not just one at a time), so it
// needs the same "images saved before this field existed just default to
// no regions" treatment rather than only fixing them up one at a time as
// they happen to be opened.
function getImages() {
  for (const r of images) if (!Array.isArray(r.lineRegions)) r.lineRegions = [];
  return images;
}
...
function addImage(rec) {
  const record = {
    ...
    lineRegions: rec.lineRegions || [],
  };
  images.push(record);
  return record;
}

// getImage() is the one place every route reads an image record through, so
// it's also the safest place to default lineRegions for images that were
// created before this field existed (older state/ files on disk) -- rather
// than a one-time migration pass, every read just self-heals.
function getImage(id) {
  const r = images.find((x) => x.id === id) || null;
  if (r && !Array.isArray(r.lineRegions)) r.lineRegions = [];
  return r;
}
...
/**
 * Line-annotation regions (the freeform "lasso" polygons drawn to fix line
 * numbers on skewed pages -- see lib/lines.js computeLineNumbers) are
 * persisted separately from annotations: they're not shapes, carry no
 * label, and are never exported as their own object -- only their EFFECT
 * (the line number they assign to whichever shapes they cover) surfaces
 * anywhere else. Same upsert-by-id pattern as replaceAnnotations, for the
 * same reason: keep each region's identity stable across saves rather than
 * generating a fresh id every time and losing continuity.
 */
function replaceLineRegions(image, incoming) {
  const byId = new Map((image.lineRegions || []).map((r) => [r.id, r]));
  image.lineRegions = incoming.map((r) => {
    const prev = r.id ? byId.get(r.id) : null;
    return {
      id: prev ? prev.id : r.id || newId(),
      points: Array.isArray(r.points) ? r.points.map((p) => ({ x: p.x, y: p.y })) : [],
      line: typeof r.line === "number" && r.line >= 1 ? Math.floor(r.line) : 1,
    };
  });
  saveImages();
}
...
module.exports = {
  ...
  getImages, getImage, addImage, replaceAnnotations, replaceLineRegions, backfillAnnotations,
