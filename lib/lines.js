...
(function (root, factory) {
  ...
  function computeLineNumbers(annotations, lineRegions) {
    ...
    // Effective line = region ?? manual ?? auto; sequence = left-to-right
    // within line.
    const byLine = new Map();
    for (const s of shapes) {
      const line = s.regionLine != null ? s.regionLine : (s.manual != null ? s.manual : autoById.get(s.id));
    ...
    for (const [line, bucket] of byLine) {
      ...
      bucket.forEach((s, i) => {
        result.set(s.id, {
          line, seq: i + 1, autoLine: autoById.get(s.id),
